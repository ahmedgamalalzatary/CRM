...
function Products() {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className={`grid grid-cols-1 md:grid-cols-4 gap-4 transition-all duration-500 ease-in-out ${animateItems ? 'opacity-100' : 'opacity-0 translate-y-4'}`} style={{ transitionDelay: '100ms' }}>
                {[
                    { name: 'All Products', count: products.length, color: 'bg-blue-100 text-blue-600 border-blue-200' },
                    { name: 'Electronics', count: products.filter(p => p.category === 'Electronics').length, color: 'bg-green-100 text-green-600 border-green-200' },
                    { name: 'Furniture', count: products.filter(p => p.category === 'Furniture').length, color: 'bg-purple-100 text-purple-600 border-purple-200' },
                    { name: 'Clothing', count: products.filter(p => p.category === 'Clothing').length, color: 'bg-yellow-100 text-yellow-600 border-yellow-200' }
                ].map((category, index) => (
                    <Card 
                        key={category.name} 
                        hover 
