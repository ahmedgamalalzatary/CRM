                    ...
                    <div className="p-6">
                        ...
                        <div className="mt-6 pt-6 border-t">
                            <div className="flex justify-between items-center mb-4">
                                <h4 className="font-medium text-gray-800">{t('Support.RecentTickets')}</h4>ticket.status === 'In Progress' ? 'bg-yellow-100 text-yellow-800' :
                                <Button variant="secondary" className="text-sm">{t('Support.ViewAll')}</Button>
                            </div>
                            <div className="overflow-x-auto">
                                <table className="min-w-full divide-y divide-gray-200">{ticket.status}
                                    <thead>
                                        <tr>
                                            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">lassName="px-4 py-3 whitespace-nowrap text-sm text-gray-700">{ticket.date}</td>
                                            ...
                                            </th>            {/* Contact Banner - Enhanced for visibility */}
                                        </tr>
                                    </thead>o-r from-blue-700 to-indigo-800 rounded-lg shadow-lg p-8 text-white
                                    <tbody className="bg-white divide-y divide-gray-200">
                                        {[
                                            { 
                                                id: 'T-1234', 
                                                subject: 'Issue with order processing',   style={{ transitionDelay: '400ms' }}
                                                status: t('Support.TicketStatuses.InProgress'), 
                                                date: '2023-01-15'    <div className="flex flex-col md:flex-row justify-between items-center">
                                            },md:border-opacity-30">
                                            { 
                                                id: 'T-1235', 
                                                subject: 'API connection error', 
                                                status: t('Support.TicketStatuses.Resolved'), assName="text-blue-100 text-lg max-w-md leading-relaxed">
                                                date: '2023-01-12' estions
                                            },
                                            { 
                                                id: 'T-1236', lassName="flex flex-col sm:flex-row gap-4 md:ml-8">
                                                subject: 'Account access problem', 
                                                status: t('Support.TicketStatuses.Closed'), iant="light"
                                                date: '2023-01-10' 
                                            }="whitespace-nowrap bg-white text-blue-600 hover:bg-blue-50 
                                        ].map((ticket, idx) => (-all 
                                            <tr key={idx} className="hover:bg-gray-50 transition-colors">
                                                <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900">
                                                    {ticket.id}
                                                </td>
                                                <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-700">   Start Live Chat
                                                    {ticket.subject}
                                                </td>
                                                <td className="px-4 py-3 whitespace-nowrap text-sm">iant="secondary"
                                                    <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${
                                                        ticket.status === t('Support.TicketStatuses.InProgress') ? 'bg-yellow-100 text-yellow-800' : ="whitespace-nowrap border-2 border-white bg-transparent 
                                                        ticket.status === t('Support.TicketStatuses.Resolved') ? 'bg-green-100 text-green-800' : 3
