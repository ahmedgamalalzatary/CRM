...
function Inventory() {
    ...
    const inventoryStats = [
        { title: 'Total Stock', value: totalStock.toLocaleString(), icon: MdWarehouse, color: 'bg-blue-100 text-blue-800' },
        { title: 'In Transit', value: '150', icon: MdLocalShipping, color: 'bg-yellow-100 text-yellow-800' },
        { title: 'Low Stock', value: alertCount.toString(), icon: MdWarning, color: 'bg-red-100 text-red-800' },
    ];
                    ...
                    <tr key={item.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                            <span className={`px-2 py-1 rounded-full text-xs ${
                                item.quantity <= item.minQuantity ? 'bg-red-100 text-red-800' :
                                item.quantity <= item.minQuantity * 2 ? 'bg-yellow-100 text-yellow-800' :
                                'bg-green-100 text-green-800'
                            }`}>
                                {item.quantity <= item.minQuantity ? 'Low Stock' :
                                 item.quantity <= item.minQuantity * 2 ? 'Medium Stock' : 'In Stock'}
                            </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
    ...
    return (
        <div className="space-y-6">
            ...
            `}>
                <h2 className="text-2xl font-semibold text-gray-800">{t('inventory.title', 'Inventory Management')}</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <Select
                    ...
                    label={t('inventory.filter', 'Filter')}
                    options={[
                        { value: 'all', label: t('all', 'All Items') },
                        { value: 'inStock', label: t('inventory.status.inStock', 'In Stock') },
                        { value: 'lowStock', label: t('inventory.status.lowStock', 'Low Stock') },
                        { value: 'outOfStock', label: t('inventory.status.outOfStock', 'Out of Stock') }
                    ]}
                />
                <Select
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <StatsCard
                    title={t('inventory.currentStock', 'Current Stock')}
                    value={totalStock}
                />
                <StatsCard
                    title={t('inventory.stockValue', 'Stock Value')}
                    value={formatCurrency(stockValue)}
                />
                <StatsCard
                    title={t('inventory.alerts', 'Alerts')}
                    value={alertCount}
                    trend="warning"
                />
                <StatsCard
                    title={t('inventory.stockMovement', 'Stock Movement')}
                    value={stockMovement}
